...
if(_state==BATTLE_STATE.MENU){
	...
	if(_menu==BATTLE_MENU.BUTTON){
		...
		with (but){
			event_user(15);
			x=battle_board.x-battle_board.left-5+28;
			y=battle_board.y-battle_board.up-5+20;
		}
	...
	if(_menu==BATTLE_MENU.FIGHT_TARGET){
		//上/下
		if (ColorDyes_switch){
			if (!instance_exists(battle_fight_menu_p3)){
				instance_create_depth(0, 0, 0, battle_fight_menu_p3);
			}
		}else{
		...
		if(Input_IsPressed(INPUT.CANCEL)){
			...
			but = Battle_SetDialog(Battle_GetMenuDialog());
			instance_destroy(battle_fight_menu_p3);
		}
		//确定
		if(Input_IsPressed(INPUT.CONFIRM)){
			audio_play_sound(snd_menu_confirm,0,false);
	...
	if(_menu==BATTLE_MENU.ITEM){
		...
		battle_soul.y=battle_board.y-battle_board.up-5+36+32+30;
	}else
	...
	if(_menu==BATTLE_MENU.MERCY){
		...
		if(Input_IsPressed(INPUT.UP)){
			...
			var mercy = Battle_GetMenuChoiceMercy()+2;
			if (mercy <= 0){
				audio_play_sound(snd_menu_switch,0,false);
				Battle_SetMenuChoiceMercy(mercy);
			}
		...
		//灵魂位置
		battle_soul.x=battle_board.x-battle_board.left-5+40*(Battle_GetMenuChoiceMercy() == 2 ? _menu_choice_mercy : 1);
		battle_soul.y=battle_board.y-battle_board.up-5+36+32*(Battle_GetMenuChoiceMercy() != 2 ? _menu_choice_mercy : 1);
...
if(_state==BATTLE_STATE.DIALOG){
	...
	if(!instance_exists(_dialog[0])){
		if(!Dialog_IsEmpty()){
			but1 = Battle_SetDialog(Dialog_Get()+"{pause}{end}");
