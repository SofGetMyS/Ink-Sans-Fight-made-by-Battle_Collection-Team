...
switch(cmd[|0]){
	case "speed":
		...
		break;
	case "repeat": 
		//if (is_real(cmd[|1]) && cmd[|1] >= 1){ _char_per_frame = cmd[|1]; }; break;
		if (is_string(cmd[|1])){
			switch(cmd[|1]){
				case "frame": if (is_real(cmd[|2]) && cmd[|2] >= 1) { _char_per_frame = cmd[|2]; }; break;
				case "voice": if (is_real(cmd[|2]) && cmd[|2] >= 1) { _voice_repeat = cmd[|2]; }; break;
			}
		}
		break;
	case "char_y": if (is_real(cmd[|1])){ _char_y *= cmd[|1]; }; break;
	case "color":
		if(is_string(cmd[|1])){
	...
	case "variable":
        if (is_string(ds_list_find_value(cmd, 1)) && is_string(ds_list_find_value(cmd, 2)))
            variable_instance_set(asset_get_index(ds_list_find_value(cmd, 1)), ds_list_find_value(cmd, 2), ds_list_find_value(cmd, 3));
        break;
}